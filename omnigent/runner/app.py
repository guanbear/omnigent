...
def create_runner_app(
    *,
    ...
    async def delete_session(session_id: str) -> JSONResponse:
        ...
        # Bump (not pop) the generation so any in-flight fill that captured the
        # pre-delete generation cannot re-populate a deleted session's caches.
        # Popping would reset to 0 and let a stale fill's generation==0 match.
    ...
    async def _resolve_session_spec_entry(session_id: str) -> _SpecEntry | None:
        ...
        async with lock:
            if session_id in _session_spec_cache:
                return _session_spec_cache[session_id]
            # Capture generation before the first await so a reset that runs
            # while we await the snapshot/resolver doesn't pin stale data.
            _spec_fill_gen = _session_snapshot_generation.get(session_id, 0)
            snapshot = await _session_snapshot(session_id)
            if not snapshot.ok:
                raise OmnigentError(
            ...
            if sub_agent_name:
                ...
                if _unwrap_resolved_spec(spec_entry) is not None:
                    ...
                    if sub_entry is None:
                        ...
                        _session_sub_agent_resolved[session_id] = True
            # Only cache if the generation hasn't moved (no reset while awaiting).
            if _session_snapshot_generation.get(session_id, 0) == _spec_fill_gen:
                _session_spec_cache[session_id] = spec_entry
            return spec_entry
